...
def ObjectLoss_evaluate(test_dataloader, generator, labels_list, videos, dataset, device, frame_height=256, frame_width=256, config={}): 
    #init
    psnr_list = {}
    roi_psnr_list = {}
    ...
    # test
    generator.eval()
    for k, (frames, objects, bboxes, flow, objects_flow) in enumerate(tqdm(test_dataloader, desc='test', leave=False)):
        ...
        frames = frames.cuda()
        flow = flow.cuda()
        rois = torch.tensor(bboxes).unsqueeze(0).cuda()
        ...
        if min(objects.shape) != 0:
            ...
            psnr_list[ sorted(videos.keys())[video_num] ].append(-1)
    ...
    for video_name in sorted(videos.keys()):
        if dataset == "ShanghaiTech":
            video_bboxes = np.load(os.path.join(config['test_bboxes_path'] ,"{}.npy".format(video_name)), allow_pickle=True)
