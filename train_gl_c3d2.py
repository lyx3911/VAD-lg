...
def train(config, weight):
    ...
    netG = "resnet_attention2"
    norm = 'instance3d'
    dropout = True
    init_type = 'normal'
    init_gain = 0.02
    gpu_ids = []
    model = define_G(netG, train_dataset_args['c'], train_dataset_args['c'], ngf=ngf,norm=norm, use_dropout=dropout, 
    ...
    for epoch in range(config['epochs']):
        ...
        model.train()
        for j, (frames, objects, bbox, flow, objects_flow) in enumerate(tqdm(train_dataloader, desc='train', leave=False)):
            frames = frames.cuda()
            flow = flow.cuda()
            rois = torch.tensor(bbox).unsqueeze(0).cuda()
            objects_flow = objects_flow.cuda()
            input = frames[:, :-1, ]
            target = frames[:, -1, ]
            if min(objects.shape) != 0:
                objects = objects.cuda()
                outputs, object_out, flow_out = model(input, objects, rois, objects_flow)
                ...
                objects_loss = int_loss(objects, object_out)
                g_loss = lam_gd * g_gd_loss + lam_int * g_object_loss + flow_loss + objects_loss             
            ...
            # break
            if config['train_dataset_type'] == "ShanghaiTech":
                if j % 15000 == 0 and j != 0: # testing
                    utils.log('----------------------------------------')
                    ...
                    frame_AUC,  psnr_list = ObjectLoss_evaluate(test_dataloader, model, labels_list, videos, dataset=config['test_dataset_type'],device = device,
                        frame_height = train_dataset_args['h'], frame_width=train_dataset_args['w'], config=config) 
