...
def define_G(netG, input_nc, output_nc, ngf, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02,
    ...
    elif netG == "C3D_attention2":
        net = C3DAttentionGenerator2(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout)
    elif netG == "resnet_attention2":
        net = ResnetAttentionGenerator2(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=6)
...
class ResnetAttentionGenerator2(nn.Module):
    """Resnet-based generator that consists of Resnet blocks between a few downsampling/upsampling operations.
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6,
        ...
        """
        assert (n_blocks >= 0)
        super(ResnetAttentionGenerator2, self).__init__()
        if type(norm_layer) == functools.partial:
            use_bias = norm_layer.func == nn.InstanceNorm2d
        else:
            use_bias = norm_layer == nn.InstanceNorm2d
        ...
        # flow feature extractor
        self.flow_feature_extractor = FlowFeatureExtractor(input_nc=2, output_nc=2)
        self.object_model = FlowFeatureExtractor(input_nc=3, output_nc=3)
    ...
    def forward(self, frames, objects, rois, flows):    # frames/objects: N x T x c x h x w，roi为目标的坐标
        ...
        object_feat, object_out = self.object_model(objects) # [N, c, h'', w'']
        # print("object_feat.shape: ", object_feat.shape)
        object_feat = self.GAP(object_feat).squeeze(-1).squeeze(-1)  # [N, c]
        # print("object_feat.shape: ", object_feat.shape)
        ...
        z = z.reshape(-1, T, z.shape[-3], z.shape[-2], z.shape[-1])
        out_recurrent,_ = self.recurrent_model(z)
        out_recurrent = out_recurrent[-1]
        # print("out_recurrent.shape: ", out_recurrent.shape) #[bs, 4, 256, 64, 64]
        out_med_feat = out_recurrent[:,int(T/2)].view(out_recurrent.shape[2], -1)   # [c=256, d1=4096]
        attention_map = self.cal_attention(out_med_feat, box_feature_aggr)
        out_recurrent = attention_map * out_recurrent
        out_recurrent = out_recurrent.reshape(out_recurrent.shape[0], -1, out_recurrent.shape[-2], out_recurrent.shape[-1])
        # print("out_recurrent.shape: ", out_recurrent.shape) #[bs, 1024, 64, 64]        
        
        out = self.de_model(out_recurrent)
        # print(out.shape)

        return out, object_out, flow_out
...
class C3DAttentionGenerator2(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm3d, use_dropout=False, n_blocks=6,
                 padding_type='zero', step=4):
        """Construct a Resnet-based generator
        ...
        """
        assert (n_blocks >= 0)
        super(C3DAttentionGenerator2, self).__init__()
        if type(norm_layer) == functools.partial:
            use_bias = norm_layer.func == nn.InstanceNorm3d
        else:
            use_bias = norm_layer == nn.InstanceNorm3d
    ...
    def forward(self, frames, objects, rois, flows):    # frames/objects: N x T x c x h x w，roi为目标的坐标
        ...
        frame_feat = self.model(frames)   
        # print(frame_feat.shape)
        frame_feat = torch.nn.functional.avg_pool3d(frame_feat, kernel_size=(4,1,1)) 
        # print("frame_feat.shape", frame_feat.shape) # [bs=1, c=256, 1, h=64, w=64]
        frame_feat = frame_feat.squeeze(2) #[bs=1,c=256, h=64, w=64]
        # frame_feat = torch.mean(frame_feat, dim=2)# [1, 256, 64, 64] # mixed 

        objects = objects.view(-1, objects.shape[-3], objects.shape[-2], objects.shape[-1]) #[N, c, w, h]
        object_feat, object_out = self.object_model(objects) # [N, 256, 16, 16]
        # print("object_out.shape: ", object_out.shape)
        # print("object_feat.shape: ", object_feat.shape) #[N, 256, 4, 16, 16]
        # object_feat = torch.mean(object_feat, dim=2) # [N, 256, 16, 16]
        object_feat = self.GAP(object_feat).view(object_feat.shape[0], object_feat.shape[1]) # [N, 256]
        # print("object_feat.shape: ", object_feat.shape)
        ...
        flow_feat = self.GAP(flow_feat).squeeze(-1).squeeze(-1)
        # print("flow_feat.shape: ", flow_feat.shape)
        # print("flow out.shape: ", flow_out.shape)
