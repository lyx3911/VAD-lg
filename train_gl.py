...
def train(config, weight):
    ...
    init_gain = 0.02
    gpu_ids = []
    model = define_G(train_dataset_args['c'], train_dataset_args['c'], ngf=ngf,norm=norm, use_dropout=dropout, 
    ...
    for epoch in range(config['epochs']):
        ...
        # Save the model
        if epoch*len(train_dataset)+j % save_epoch == 0 or epoch == config['epochs'] - 1:
